...
;; Package-Version: 20251209.2022
;; Package-Revision: 1800a4e367c2
...
(defun restclient--multipart-part (name value boundary)
  (let ((eol "\r\n"))
    (if (string-prefix-p "@file:" value)
        ;; ===== file part =====
        (let* ((path (string-remove-prefix "@file:" value))
               (filename (file-name-nondirectory path))
               (mime (or (mailcap-extension-to-mime
      ...
      (concat
       ...
       eol
       value eol))))
(defun restclient-build-multipart (parts boundary)
  (let ((eol "\r\n"))
    (mapconcat
     #'identity
     (append
      (mapcar (lambda (part)
                (restclient--multipart-part
                 (car part) (cdr part) boundary))
              parts)
...
(defun restclient-parse-multipart-dsl (body)
  (let (result)
    ...
    (when (re-search-forward restclient-method-url-regexp (point-max) t)
      (let* ((vars (restclient-find-vars-before-point))
        ...
        (let* ((cmax (restclient-current-max))
               (raw-body (buffer-substring (min (point) cmax) cmax))
               (boundary nil)
               (entity
                (if (string-match "^@multipart" raw-body)
                    (progn
                      (setq boundary (format "----restclient-%x" (random)))
                      (setq headers
                            (cons
                             (cons "Content-Type"
                                   (concat "multipart/form-data; boundary=" boundary))
                             headers))
                      (restclient-build-multipart
                       (restclient-parse-multipart-dsl
                        (replace-regexp-in-string "^@multipart\\|^@end" "" raw-body))
                       boundary))
                  (restclient-parse-body raw-body vars))))
